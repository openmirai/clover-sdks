...
public final class CloverClient {
  ...
  public static final class CloverException extends RuntimeException {
    ...
    CloverException(int status, String message, Map<String, Object> problem, ResponseMeta metadata) { super(message); this.status = status; this.problem = problem; this.metadata = metadata; }
  ...
  private static final class ResponseBodyLimitException extends IOException {
    ...
    ResponseBodyLimitException(int status, Map<String, String> headers) { super("Clover response body exceeds the configured limit"); this.status = status; this.headers = headers; }
  }
  ...
  public Map<String, Object> send(Map<String, Object> request, String idempotencyKey) throws IOException, InterruptedException { return request("POST", "/v1/emails", request, idempotencyKey); }
  public Map<String, Object> sendBatch(List<Map<String, Object>> items, String idempotencyKey) throws IOException, InterruptedException { return request("POST", "/v1/emails/batch", Map.of("items", items.stream().map(item -> { Map<String, Object> copy = new LinkedHashMap<>(item); copy.remove("scheduled_at"); return copy; }).toList()), idempotencyKey); }
  public Map<String, Object> schedule(String id, String scheduledAt, String idempotencyKey) throws IOException, InterruptedException { return request("POST", "/v1/emails/" + pathSegment(id) + "/schedule", Map.of("scheduled_at", scheduledAt), idempotencyKey); }
  public Map<String, Object> cancel(String id, String idempotencyKey) throws IOException, InterruptedException { return request("POST", "/v1/emails/" + pathSegment(id) + "/cancel", null, idempotencyKey); }
  public Map<String, Object> get(String id) throws IOException, InterruptedException { return request("GET", "/v1/emails/" + pathSegment(id), null, null); }
  public Map<String, Object> list(Map<String, String> filters) throws IOException, InterruptedException { StringBuilder query = new StringBuilder(); if (filters != null) for (Map.Entry<String, String> entry : filters.entrySet()) query.append(query.length() == 0 ? '?' : '&').append(java.net.URLEncoder.encode(entry.getKey(), java.nio.charset.StandardCharsets.UTF_8)).append('=').append(java.net.URLEncoder.encode(entry.getValue(), java.nio.charset.StandardCharsets.UTF_8)); return request("GET", "/v1/emails" + query, null, null); }
